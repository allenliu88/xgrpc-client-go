...
func CreateClient(clientName string, connectionType ConnectionType, labels map[string]string, xgrpcServer *xgrpc_server.XgrpcServer) (IRpcClient, error) {
	cMux.Lock()
	defer cMux.Unlock()
	if _, ok := clientMap[clientName]; !ok {
		var rpcClient IRpcClient
		if GRPC == connectionType {
			rpcClient = NewGrpcClient(clientName, xgrpcServer)
		}
		if rpcClient == nil {
		...
		for {
			...
			case rc := <-r.reconnectionChan:
				if (rc.serverInfo != ServerInfo{}) {
					...
					for _, v := range r.xgrpcServer.GetServerList() {
			...
			case <-timer.C:
				r.healthCheck(timer)
			case <-r.xgrpcServer.ServerSrcChangeSignal:
...
func (r *RpcClient) notifyServerSrvChange() {
	...
	var found bool
	for _, ele := range r.xgrpcServer.GetServerList() {
...
func (r *RpcClient) reconnect(serverInfo ServerInfo, onRequestFail bool) {
	...
	for !r.isShutdown() {
		...
		if reConnectTimes > 0 && reConnectTimes%len(r.xgrpcServer.GetServerList()) == 0 {
...
func (r *RpcClient) sendHealthCheck() bool {
	...
	if !response.IsSuccess() {
		// when client request immediately after the xgrpc server starts, the server may not ready to serve new request
...
func (r *RpcClient) nextRpcServer() (ServerInfo, error) {
	serverConfig, err := r.xgrpcServer.GetNextServer()
	if err != nil {
