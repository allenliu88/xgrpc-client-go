...
func TestNewClientConfigWithOptions(t *testing.T) {
	config := NewClientConfig(
		...
		WithEndpoint("http://console.xgrpc.io:80"),
		WithLogLevel("error"),
		WithBeatInterval(int64(2000)),
		WithUpdateThreadNum(30),

		WithLogDir("/tmp/xgrpc/log"),
		WithCacheDir("/tmp/xgrpc/cache"),
	...
	assert.Equal(t, config.Endpoint, "http://console.xgrpc.io:80")
	...
	assert.Equal(t, config.LogDir, "/tmp/xgrpc/log")
	assert.Equal(t, config.CacheDir, "/tmp/xgrpc/cache")

	assert.Equal(t, config.NotLoadCacheAtStart, true)
	assert.Equal(t, config.UpdateCacheWhenEmpty, true)

	assert.Equal(t, config.Username, "xgrpc")
	assert.Equal(t, config.Password, "xgrpc")
